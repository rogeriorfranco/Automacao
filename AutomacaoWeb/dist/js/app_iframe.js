...
var addTabsDom=function(options){
    ...
    if (findTabPanel(pageId) == null) {
        ...
        if (options.content) {
            ...
            //没有内容，使用IFRAME打开链接
            //此处src使用懒加载
            var $iframe = $("<iframe></iframe>").attr("data-src", options.url).css("width", "100%").attr("frameborder", "no").attr("id", "iframe_" + pageId).addClass("tab_iframe").attr(pageIdField, pageId);
...
//缓存标签页
function TabCache(){
	this.tabCache_key="tabCache_key";
	//获取缓存对象
...
TabCache.prototype = {
		//写入
		write:function(){
			localStorage.setItem(this.tabCache_key,JSON.stringify(this.cache));
		},
		//读取
		read:function(){
			return JSON.parse(localStorage.getItem(this.tabCache_key))||[];
		},
		//选项卡添加到缓存
		put:function(options){
			var pageId=options.id;
			//如果缓存中已经存在
			if(options.title=="我的桌面"||this.get(pageId)){
				return;
		    }
			this.cache.push(options);
			this.write();
		},
		//获取缓存选项卡
		get:function(id){
			for(var i=this.cache.length;i>-1;i--){
				var options=this.cache[i];
