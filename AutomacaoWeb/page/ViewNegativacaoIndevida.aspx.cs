...
{
    public partial class ViewNegativacaoIndevida : System.Web.UI.Page
    {
        private async Task populaCampos()
        {
            try
            {
                if (!string.IsNullOrEmpty(Request.QueryString["id_caso"]))
                {
                    ...
                    form = await formularioBll.readAsync(id_caso, (int)Util.TFormulario.NegativacaoIndevida);
                    if (form != null)
                    {
                        ...
                        NegativacaoIndevidaBLL negIndevidaBll = new NegativacaoIndevidaBLL();
                        var f = await negIndevidaBll.read(form.id);
                        if (f != null)
                        {
                        ...
                        UserControlExecQuestao1.tipoFormulario = Convert.ToInt16(Util.TFormulario.NegativacaoIndevida);
                        UserControlQuestao5.tipoFormulario = Convert.ToInt16(Util.TFormulario.NegativacaoIndevida);
        ...
        private void update(NegativacaoIndevida f)
        {
            try
            {
                FormularioBll formularioBll = new FormularioBll();
        ...
        {
            NegativacaoIndevida f = new NegativacaoIndevida();
