...
{
    public partial class ViewProblemasTecnicos : System.Web.UI.Page
    {
        private async Task populaCampos()
        {
            try
            {
                if (!string.IsNullOrEmpty(Request.QueryString["id_caso"]))
                {
                    ...
                    form = await formularioBll.readAsync(id_caso, (int)Util.TFormulario.ProblemasTecnicos);
                    ...
                    if (form != null)
                    {
                        ...
                        ProblemaTecnicoBLL probTecnicoBll = new ProblemaTecnicoBLL();
                        var f = await probTecnicoBll.read(form.id);
                        ...
                        UserControlExecQuestao1.tipoFormulario = Convert.ToInt16(Util.TFormulario.ProblemasTecnicos);
                        UserControlQuestao5.tipoFormulario = Convert.ToInt16(Util.TFormulario.ProblemasTecnicos);
        ...
        private void update(ProblemaTecnico f)
        {
            try
            {
                FormularioBll formularioBll = new FormularioBll();
                ProblemaTecnicoBLL problemaTecnicoBLL = new ProblemaTecnicoBLL();
        ...
        {
            ProblemaTecnico p = new ProblemaTecnico();
            p.preenchimentoCompleto = false;
            update(p);
        }
