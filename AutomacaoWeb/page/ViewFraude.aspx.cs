...
{
    public partial class ViewFraude : System.Web.UI.Page
    {
        private async Task populaCampos()
        {
            try
            {
                if (!string.IsNullOrEmpty(Request.QueryString["id_caso"]))
                {
                    ...
                    form = await formularioBll.readAsync(id_caso, (int)Util.TFormulario.Fraude);
                    ...
                    if (form != null)
                    {
                        ...
                        var f = await fraudeBll.read(form.id);
                        ...
                        UserControlExecQuestao1.tipoFormulario = Convert.ToInt16(Util.TFormulario.Fraude);
                        UserControlQuestao5.tipoFormulario = Convert.ToInt16(Util.TFormulario.Fraude);
                        ...
                        await UserControlQuestao10.populaCampos(form.id, form.status, form.tipo_demanda);
        ...
        private void update(Fraude f)
        {
            try
            {
                ...
                if (UserControlQuestao10.panelPrincipal10.Visible)
                    UserControlQuestao10.Salvar(f.formularioId);
