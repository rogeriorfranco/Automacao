...
{
    public partial class ViewRedeCobertura : System.Web.UI.Page
    {
        private async Task populaCampos()
        {
            try
            {
                if (!string.IsNullOrEmpty(Request.QueryString["id_caso"]))
                {
                    ...
                    form = await formularioBll.readAsync(id_caso, (int)Util.TFormulario.RedeCobertura);
                    ...
                    if (form != null)
                    {
                        ...
                        var f = await redeCoberturaBLL.read(form.id);
        ...
        private void update(RedeCobertura f)
        {
            try
            {
                FormularioBll formularioBll = new FormularioBll();
        ...
        {
            RedeCobertura r = new RedeCobertura();
            r.preenchimentoCompleto = false;
            update(r);
        }
